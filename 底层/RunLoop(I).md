...
- [**RunLoop**](#RunLoop)
	...
	- [启动子线程的 RunLoop](#启动子线程的RunLoop)
- [**NSRunLoop**](#NSRunLoop)
- [**RunLoop的 C 语言的的结构体 CFRunLoopRef**](#RunLoop的C语言的的结构体CFRunLoopRef)
	- [RunLoop Mode](#RunLoopMode)
...
&emsp; 广义上的来说，`run loop` 就是所谓的 event loop，或者称之为`「事件循环」`或者「`事件分发器」`。Event loop 是 event-driven programming（事件驱动编程）非常重要的组成部分，而事件驱动编程则是 `GUI 程序`的最常见编程方式（现在似乎在服务器端也有很多应用，但在 GUI 编程方面肯定是绕不过去）。
...
><h2 id='Runloop与线程的联系'>Runloop与线程的联系</h2>
...
><h2 id='RunLoop在OC中有获取'>RunLoop在OC中有获取</h2>
...
><h2 id='启动子线程的RunLoop'>启动子线程的 RunLoop</h2>
...
><h1 id='RunLoop的C语言的的结构体CFRunLoopRef'>RunLoop的 C 语言的的结构体 CFRunLoopRef</h1>
...
> <h1 id='RunLoopSource'>RunLoop Source</h1>
...
> <h1 id='CFRunLoopObserverRef'>CFRunLoopObserverRef</h1>
...
> <h1 id='CFRunLoopTimerRef'>CFRunLoopTimerRef</h1>
