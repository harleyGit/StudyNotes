...
> <h2 id='Consumer组件使用'>Consumer组件使用</h2>
...
&emsp; Consumer是一个非常实用的组件，它来自于provider包，这是一个状态管理库。Provider库允许你以一种高效且简洁的方式在Widget树中管理及传递状态。Consumer组件扮演了一个监听者或者订阅者的角色，它能够监听某个Provider所管理的状态变化，并在状态改变时重建自身及其子树，从而实现UI的更新。

&emsp; 当你将Consumer包裹在你的Widget树的某一部分时，你可以指定它应该监听哪个类型的Provider，并且提供一个回调函数（通常命名为builder），在这个回调中你可以访问到最新的状态数据，并基于此数据构建或重构建UI。这样做的好处是，只有当监听的状态发生变化时，相关的UI部分才会被重建，从而提高了应用程序的性能和效率。
...
> <h2 id='Consumer内部实现'>Consumer内部实现</h2> 
...
[**看这里**](https://book.flutterchina.club/chapter7/value_listenable_builder.html)
