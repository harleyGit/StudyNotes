...
这行代码的本质是：**从字符串中提取所有数字字符，如果 `text` 为 `nil` 就返回空字符串。**
...
`filter` 会遍历字符串中的每一个字符，只保留满足条件的字符。`$0` 表示当前遍历到的 `Character`，`isNumber` 表示是否为数字字符。
...
`Character.isNumber` 不只包含 `0~9`，也可能包含一些 Unicode 数字。
...
`text?.filter(...)` 是可选链：如果 `text` 有值，就执行 `filter`；如果 `text == nil`，整个表达式直接返回 `nil`，不会崩溃。

`?? ""` 是 nil 合并运算符：如果前面的结果是 `nil`，就返回空字符串。
...
如果明确只需要 ASCII 数字，可以写成：

```swift
text?.filter { $0 >= "0" && $0 <= "9" } ?? ""
```

这种写法更严格，不受 Unicode 数字影响，性能也略好。
...
就是：**从可选字符串中提取所有数字；如果字符串为空，就返回空字符串。**
