...
&emsp; 完成属性定义后，编译器会自动编写访问这些属性所需的方法，此过程叫做“自动合成”(autosynthesis)。需要强调的是，这个过程由编译 器在编译期执行，所以编辑器里看不到这些“合成方法”(synthesized method)的源代码。除了生成方法代码 getter、setter 之外，编译器还要自动向类中添加适当类型的实例变量，并且在属性名前面加下划线，以此作为实例变量的名字。

&emsp; 我们每次在增加一个属性,系统都会在 ivar_list 中添加一个成员变量的描述,在 method_list 中增加 setter 与 getter 方法的描述,在属性列表中增加一个属性的描述,然后计算该属性在对象中的偏移量,然后给出 setter 与 getter 方法对应的实现,在 setter 方法中从偏移量的位置开始赋值,在 getter 方法中从偏移量开始取值,为了能够读取正确字节数,系统对象偏移量的指针类型进行了类型强转.
...
&emsp;  比如:上述的_name的地址是0x1111,它的指针和name的指针指向同一个地方,是指针的拷贝.
...
- (void)setName:(NSString *)name{
    if (_name != name) {
        [ _name release];//name的地址为:0x1111
        _name = [name copy];//name的地址为:0x2322,当然得是NSMutableNsstring.若是NSString指针地址是不变的
    }
}
- (NSString *)name{
    return [[ _name retain] autorelease];
...
- 1.当我们使用这个关键字 _weak 时就开始用到了弱引用的一些底层方面的实现了,其实这里也是通过 objc_msgSend方法进行消息的发送的.调用的方法是**objc_initWeak**
...
- id *location ：__weak指针的地址，即例子中的weak指针取地址： &weakObj 。它是一个指针的地址。之所以要存储指针的地址，是因为最后我们要讲__weak指针指向的内容置为nil，如果仅存储指针的话，是不能够完成这个功能的。
- id newObj ：所引用的对象。即例子中的obj 。

有一个返回值 id ： 会返回obj自身，但其值已经做了更改（isa_t中的weak_ref位置1）

<br/>

- 2.这里通过objc的地址得到其引用对象referent

- 3.有好多张散列表SideTables,记得好像是64个.通过这个对象objc找到在这些SideTables中的某一个新的SideTable(新表);
	- 通过location(weak指针地址)找到的表是旧的SideTable(旧表)
...
> <h2 id='子类重写父类属性'>子类重写父类属性</h2>

&emsp; 在 Objective-C 中，如果你有一个父类拥有的属性是 id 类型，而你想在子类中将其修改为具体的某一类型属性，你可以使用属性的覆盖（override）来实现这个目标。以下是实现的步骤：

在父类中声明属性为 id 类型：

在父类中，你可以声明属性为 id 类型，如下所示：
...
在子类中，你可以覆盖这个属性，将其修改为具体的某一类型。为此，你需要重新声明属性，并使用具体的类型，如下所示：

```
@interface ChildClass : ParentClass
@property (nonatomic, strong) NSString *myProperty; // 具体的类型
@end
```


请注意，子类中的属性名必须与父类中的属性名相同，以确保覆盖。

实现 Setter 和 Getter 方法：

在子类中，你还可以实现自己的 setter 和 getter 方法来处理这个属性。例如：
...
@end
```
这允许你在设置或获取属性时执行自定义操作。

通过这种方式，你可以在子类中将父类的 id 属性修改为具体的某一类型属性，并可以在子类中添加自己的逻辑。需要注意的是，一旦你在子类中覆盖了属性，子类的实例将会使用子类的属性类型，而不再是父类的属性类型。
...
可以借助@synthesize和@dynamic关键字进行指定实现set、get方法进行解决:


使用 @synthesize 和 @dynamic 关键字可以在一定程度上控制属性的存取方法的生成，但无法实现属性的类型变更。在 Objective-C 中，属性的类型是由属性声明时的类型决定的，而不是通过 @synthesize 或 @dynamic 来控制的。

具体来说，使用 @synthesize 来合成存取方法时，它会合成与属性声明的类型相符的存取方法。例如，如果你的父类属性声明为 id 类型，@synthesize 会生成适用于 id 类型的存取方法。使用 @dynamic 关键字告诉编译器你将在运行时提供这些方法的实现，但它不会改变属性的类型。

如果你想在子类中将属性的类型更改为具体的某一类型，必须重新声明属性，如我在之前的回答中所示。子类的属性类型将覆盖父类的属性类型，而无需使用 @synthesize 或 @dynamic。

在实际应用中，属性的类型通常是与类的设计密切相关的，因此更改属性类型可能需要谨慎考虑，以确保不会导致运行时错误或不一致性。

<br/>
<br/>

当你在子类中没有重写属性的 get 和 set 方法，并且父类中的属性声明为 id 类型时，Xcode 可能会生成警告，提示你属性的实现将由父类来合成。这是一个警告，不一定表示错误，但它提醒你该属性的存取方法会在父类中自动生成。

如果你明确了解到这个属性会在子类中实现，而不是由父类自动生成，可以使用 @dynamic 关键字来告诉编译器你打算在子类中提供属性的实现，这样警告就会消失。

<br/>
<br/>

当你在属性声明中使用 @dynamic 关键字告诉编译器属性的实现将在运行时提供，然后你又使用 @synthesize 来合成属性的存取方法时，编译器会自动生成默认的 get 和 set 方法，不需要再显式重写它们。

示例：

```
@interface ChildClass : ParentClass
@property (nonatomic, strong) SpecificType *viewModel;
@end

@implementation ChildClass
@dynamic viewModel; // 使用@dynamic告知编译器，属性的实现将在运行时提供
@synthesize viewModel; // 使用@synthesize合成属性的存取方法，不需要再重写get和set方法
@end
```


在上述示例中，使用了 @dynamic 告诉编译器属性的实现将在运行时提供。然后，使用 @synthesize 合成了 viewModel 属性的存取方法，编译器会自动生成默认的 get 和 set 方法，你不需要再显式重写它们。

这样做的好处是，你可以将属性的实现留给运行时，但仍然使用编译器生成的默认存取方法，而无需手动编写它们。这通常是最常见的用法，除非你需要在 get 或 set 方法中添加自定义逻辑。
...
嗯呢 好的 感谢
ChatGPT
不客气！如果你有任何其他问题或需要进一步的帮助，随时都可以提问。祝你编程顺利！
