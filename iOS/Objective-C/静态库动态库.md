...
- [**语言编译流程**](#语言编译流程)
	...
	- [编译命令](#编译命令)
	- [xcconfig文件设置说明](#xcconfig文件设置说明)
- [**静态库和动态库**](#静态库和动态库)
	- [Build Settings配置](#BuildSettings配置)
		- [module方式导入和头文件导入区别](#module方式导入和头文件导入区别)
...
- **参考资料**
	...
	- [静态库和动态库区别-重要](http://quanzhan.applemei.com/webStack/TmpBeE9BPT0=)
...
![ios_oc1_113_11.jpg](./../../Pictures/ios_oc1_113_11.jpg)

![ios_oc1_113_12.jpg](./../../Pictures/ios_oc1_113_12.jpg)

![ios_oc1_113_13.jpg](./../../Pictures/ios_oc1_113_13.jpg)

![ios_oc1_113_14.jpg](./../../Pictures/ios_oc1_113_14.jpg)

![ios_oc1_113_15.jpg](./../../Pictures/ios_oc1_113_15.jpg)

![ios_oc1_113_16.jpg](./../../Pictures/ios_oc1_113_16.jpg)

![ios_oc1_113_17.jpg](./../../Pictures/ios_oc1_113_17.jpg)

![ios_oc1_113_18.jpg](./../../Pictures/ios_oc1_113_18.jpg)



<br/><br/>

> <h2 id='xcconfig文件设置说明'>xcconfig文件设置说明</h2>


![ios_oc1_113_19.jpg](./../../Pictures/ios_oc1_113_19.jpg)

![ios_oc1_113_20.jpg](./../../Pictures/ios_oc1_113_20.jpg)

![ios_oc1_113_21.jpg](./../../Pictures/ios_oc1_113_21.jpg)
...
![ios_oc1_113_10.png](./../../Pictures/ios_oc1_113_10.png)


- 如上图的config配置文件中描述:
	- HEADER_SEARCH_PATHS表示的是库文件编译以后产物的头文件路径;
		- BUILD_DIR:表示编译后产物Products所在路径;
		- CONFIGURATION:表示编译后的文件夹,比如:Debug文件夹
		- EFFECTIVE_PLATFORM_NAMW:平台名称,若是Mach-o则为空
	- FRAMEWORK_SEARCH_PATHS:库文件存放的位置;
		- 通过路径的方式,链接framework
	- OTHER_LDFLAGS:库文件的名字;


<br/><br/><br/>

> <h2 id='module方式导入和头文件导入区别'>module方式导入和头文件导入区别</h2>


不知道大家发现没有当我们在使用静态库或者动态库导入头文件时
...
时就可以在.m文件中使用我们暴露出的类了,然后实例化调用相应的方法,我们也没有进行**HEADER_SEARCH_PATHS**的相关路径配置.


这是为什么呢? 难道这个**HEADER_SEARCH_PATHS**不需要配置吗? 没有用吗?

错,这是有用的.当我们在使用的地方进行如下导入头文件的时候:

```
//头文件方式
#import <xxxFramework.h>
```

我们编译运行的是候就会报错,这是为什么呢?
...
进行导入头文件的时候,默认使用modules文件下的

![ios_oc1_113_25.png](./../../Pictures/ios_oc1_113_25.png)

映射文件,如下:

![ios_oc1_113_26.png](./../../Pictures/ios_oc1_113_26.png)

所以我们经常看到伞文件找不到**umbrella header xx not founde** 类似的错误.这种模式我们成为module方式导入头文件的.
...
这种方式是直接导入头文件的方式,所以我们要配置头文件路径如:

```
HEADER_SEARCH_PATHS = $(interited) "$(SRCROOT)/../xxxFramework.framework/Headers"
```

上面的情况在都是OC的静态库是足够的,但是我们在使用OC调用Swift代码暴露出的方法时可能出现问题.问题的原因是:我们仅暴露出了头文件,并没有暴露出一些module相关方法,所以还需要一些配置:

![ios_oc1_113_24.png](./../../Pictures/ios_oc1_113_24.png)

```
//用来给calng编译传递参数
OTHER_CFLAGS =  "-fmoudle-map-file=$(SRCROOT)/../xxxFramework.framework/Modules/module.modulemap"
```

这样才能解析里面的api.

但是我们一运行,可能还会报错是因为 **auto linked** 链接问题,这是因为我们没有使用module方式导致一些在编译的时候一些命令没有插入造成的.


所以我们这时候需要在在xcconfig进行配置:
...
在这些配置里,我们可以在cocoapods里面进行查找、学习
