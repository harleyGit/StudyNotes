...
- [**导航控制器**](#导航控制器)
	...
	- 	[改变导航栏透明状态](#改变导航栏透明状态)
- 	[**栈内视图控制器管理**](#栈内视图控制器管理)
- 	[nav.visibleViewController和tab.selectedViewController区别](#nav.visibleViewController和tab.selectedViewController区别)
- [**Hybrid**](#Hybrid)
	- [app调用H5](#app调用H5)
	- [H5调用app](#H5调用app)
- 	**App跳转**
...
> <h1 id= "nav.visibleViewController和tab.selectedViewController区别">nav.visibleViewController和tab.selectedViewController区别</h1>
...
class AppMainVC: UITabBarController, UITabBarControllerDelegate {
...
这两个属性是 **UIKit 容器控制器里最核心的“当前正在展示的子控制器”入口**，但它们语义不同、使用场景也不同。你可以把它们理解为：

<br/>

> 👉 **“当前屏幕上真正对用户可见的那个 VC” 的获取方式**
...
```text
ArgusAppMainVC (UITabBarController)
 ├── homeNav   ← 当前选中
 ├── messageNav
 └── mineNav
```
...
```swift
tab.selectedViewController == homeNav
```
...
```swift
nav.visibleViewController == ScanProcessVC
```

---
<br/>

🔥 和 `topViewController` 的区别（重点）

| 属性                      | 含义         |
| ----------------------- | ---------- |
| `topViewController`     | 栈顶 VC      |
| `visibleViewController` | 当前屏幕显示的 VC |

---
<br/>

⚠️ 为什么推荐用 `visibleViewController`？
...
```swift
nav.topViewController == B
nav.visibleViewController == C ✅
```
...
| 容器类型                 | 作用          |
| -------------------- | ----------- |
| TabBarController     | 决定“哪个模块”    |
| NavigationController | 决定“模块里的哪一页” |
...
| API                      | 本质           |
| ------------------------ | ------------ |
| `selectedViewController` | 当前激活的“功能模块”  |
| `visibleViewController`  | 当前用户正在看的“页面” |
