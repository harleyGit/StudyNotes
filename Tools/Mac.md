...
***
<br/><br/><br/>
> <h2 id="卸载Mac自带app">卸载Mac自带app</h2>

明白了！你是想卸载 **macOS 自带的系统应用**，比如"国际象棋"（Chess）、"家庭"（Home）这种，对吧？🍏

这种确实比普通 App 难删一点，因为：
- **系统应用是受保护的**（SIP，系统完整性保护，System Integrity Protection）。
- 普通方式（拖废纸篓、终端 rm -rf）**是不行的**，权限不够。

不过，要是你真的想删，有办法，但是要注意：
> ⚠️ 要**关闭 SIP**，才可以强删。关闭 SIP 有一点小风险（比如系统稳定性、安全性可能受影响），所以动手前要有心理准备。
...
1. 关机。
2. 开机时，**同时按住 Command (⌘) + R** 进入 **恢复模式**（屏幕出现苹果 Logo 后松手）。
3. 在恢复模式里，点顶部菜单 **实用工具** → **终端**。
4. 终端里输入：
...
为了保护你的系统安全，**建议你删完后重新开启 SIP**，否则以后安装系统更新、保护隐私会有问题。

步骤就是：
1. 再次关机。
2. 按住 `Command + R` 进恢复模式。
3. 打开终端，输入：
...
# ⚡注意：
- 有些 App 被系统组件依赖，比如“音乐”、“Safari”等，即使你删了，系统更新可能还会自己装回来……
- 非常小的、无害的自带 App（比如国际象棋）删了基本没影响。

---

好嘞～明白你想要**点点鼠标就能卸载**自带 App 的方式对吧，来，推荐给你：

---

# 🎯 推荐图形界面工具

## 1. **AppCleaner**（免费，小巧，适合普通 App）

- 主要用来卸载第三方 App（对系统自带 App 有时候权限不够）
- 下载官网：[https://freemacsoft.net/appcleaner/](https://freemacsoft.net/appcleaner/)
- 拖拽 App 进去，AppCleaner 会自动找残留文件，一键删干净。

👉 **但是**，要删系统自带 App，还是需要先关 SIP，否则 AppCleaner 也删不了。

---

## 2. **Permute with SIP disabled** + AppCleaner（终极组合）

- 关闭 SIP（前面教你的方法）
- 然后用 AppCleaner 拖自带 App，也能删。

---

## 3. **AppCleaner + Terminal 帮手**（半自动）

如果想要纯图形界面搞定，还可以加一个小工具配合，叫：

## 4. **Sensei**（收费，功能超强）

- 官网：[https://sensei.app/](https://sensei.app/)
- 支持卸载系统应用、清理垃圾、监控硬盘、电池健康、性能优化等等。
- 但 Sensei 是**付费的**（好像￥150左右终身，或者订阅制）。
- 它自带"卸载器"，可以识别出系统内置的 App（比如 Chess、Home），一键删除。

👉 如果你想"图形化 + 无脑点点删除"，**Sensei**是目前体验最好的。
...
| 需求                  | 推荐工具              | 是否需要关闭 SIP？ |
|----------------------|----------------------|--------------------|
| 普通卸载 + 免费         | AppCleaner           | 是（针对自带 App） |
| 高级全能型（付费）       | Sensei                | 否（有些可以直接删） |
