...
![go.0.0.146.png](./../Pictures/go.0.0.146.png)
...
![go.0.0.147.png](./../Pictures/go.0.0.147.png)
...
***
<br/><br/><br/>
> <h2 id="InnoDB和MyISAM引擎区别">InnoDB和MyISAM引擎区别</h2>

- InnoDB引擎: 具备外键支持功能的事务存储引擎(次要,因为外键效率低一半不会采用,主要的是事务).
	- **优点:**
		- InnoDB是MySQL的**默认事务型引擎(主要)**，它被设计用来处理大量的短期(short-lived)事务。可以确保事务的完整提交(Commit)和回滚(Rollback)。
...
- MyISAM: MyISAM提供了大量的特性，包括全文索引、压缩、空间函数(GIS)等，但MyISAM`不支持事务、行级锁、外键`，有一个毫无疑问的缺陷就是`崩溃后无法安全恢复(这是因为它不支持事务造成的)`。 
	- **优点:**
