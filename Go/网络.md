...
👉 总结：
`conns sync.Map` 就是一个 **并发安全的连接池（map）**，适合保存所有客户端连接，支持安全的存取、删除、广播。

---

要不要我给你写一个完整示例：**服务端保存多个客户端连接，客户端连接后能相互收发消息**，并展示 `sync.Map` 的实际用法？


好的 👍 我来给你写一个 `sync.Map` 的示例代码，并详细解释它的作用和使用场景。

在 Go 里，`sync.Map` 是一个**并发安全的 map**，和内置的 `map` 不同，它在多协程环境下不用额外加锁就能安全使用。
...
要不要我再帮你在这个基础上扩展一个 **广播消息给所有客户端的示例**（比如一个客户端发消息，服务端转发给所有在线用户）？这样你会更直观地看到 `sync.Map` 的作用。
